...
import { Layout, Card, Typography, Table, Button, Row, Col, Space, Divider, Descriptions, message } from 'antd';
          ...
          <Col xs={24} lg={16}>
            <Card title="Resumen de tu carrito" style={styles.card}>
                          { 
                              ...
                              <ProductRow key={prodItem.product.id} cartProduct={prodItem} /> 
...
const ProductRow = ({ cartProduct }) => (
    ...
    >
      ...
      />
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 600, fontSize: 16 }}>{cartProduct.product.name}</div>
        <div style={{ color: '#888', fontSize: 14 }}>{cartProduct.product.category.name}</div>
        <div style={{ marginTop: 4, fontSize: 14 }}>
          Cantidad: {cartProduct.quantity} x ${cartProduct.product.price.toFixed(2)}
        </div>
      </div>
